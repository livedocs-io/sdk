...
def format_single_value(value: Any, format_type: str, fixed_decimals: Optional[int] = None) -> str:
    """
    ...
    if format_type in ["number", "percent"]:
        try:
            ...
            # Format as percentage
                ...
                return f"{numeric_value * 100:.0f}%" if (numeric_value * 100).is_integer() else f"{numeric_value * 100:.2f}%"
    ...
    try:
        ...
        if compare_type == "compare_value":
            ...
            formatted_value = (
                ...
                if fixed_decimals is not None
                else f"{abs(comparison_value):.0f}" if comparison_value.is_integer()
            ...
            formatted_value = (
                ...
                if fixed_decimals is not None
                else f"{abs(comparison_value) * 100:.0f}%" if (comparison_value * 100).is_integer()
...
def process_single_value(config: str, context: Optional[dict] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        compare_format = (
            "compare_value" if raw_format == "standard"
            else "absolute_change" if raw_format == "absolute"
            else raw_format
        )
