...
def process_conditions(
    ...
    for i, condition in enumerate(filters):
        ...
        try:
            condition_mask = create_condition_mask(df, column, operator, value, column_type)
    ...
    for column in unique_columns:
        if column in filtered_df.columns:
            try:
                ...
                unique_values = counts.filter(pl.col("count") == 1).select(pl.col(column))
...
def create_condition_mask(df: pl.DataFrame, column: str, operator: str, value: Any, column_type: str) -> Optional[pl.Expr]:
    """
    ...
    if column_type == "number":
        ...
        if operator == 'true':
            return pl.col(column) == True
        elif operator == 'false':
            return pl.col(column) == False
        elif operator == 'eq':
...
def create_date_condition_mask(df: pl.DataFrame, column: str, operator: str, value: str) -> Optional[pl.Expr]:
    """
...
def _get_column_type(df: pl.DataFrame, column: str) -> str:
    ...
    try:
        ...
        if isinstance(dtype, pl.Boolean):
            return "boolean"
        elif isinstance(dtype, (pl.Int8, pl.Int16, pl.Int32, pl.Int64, pl.UInt8, 
        ...
        try:
            ...
            # Numeric calculations
                ...
                if calc_type == "Sum":
                    ...
                    value = numeric_col.value_counts().sort(by="counts", descending=True).select("values").head(1)[0, 0]
                elif calc_type == "Min":
