...
class ChartGenerator:
    ...
    def format_column_data(self, df, fmt, col):
        ...
        if col in df.columns:
            if (
                df[col].dtype == pl.Date32
                or df[col].dtype == pl.Date64
                or df[col].dtype == pl.Datetime
            ):
                if fmt == "year":
    ...
    def general_data_pre_trasformer(self, df, column_meta, chart_type):
        ...
        if primary_axis["format"]:
            date_formated_data = self.format_column_data(
                df=df, fmt=primary_axis["format"], col=primary_axis["column_name"]
            )

        filtered_data = date_formated_data
        transformed_data = date_formated_data
        if (
            primary_axis["column_name"]
            and primary_axis["filter_type"]
            and primary_axis["filter_by"]
        ):
            filtered_data = self.filter_dataframe(
                df=df,
                column=primary_axis["column_name"],
                filter_type=primary_axis["filter_type"],
                filter_value=primary_axis["filter_by"],
            )

        print(filtered_data)
        if chart_type == "pie_chart":
            transformed_data = self.aggregate_dataframe(
                df=filtered_data,
                group_by_column=secondary_axis[0]["group_by"],
                agg_dict=secondary_axis,
            )
        else:
            transformed_data = self.aggregate_dataframe(
                df=filtered_data,
                group_by_column=primary_axis["column_name"],
                agg_dict=secondary_axis,
            )
    ...
    def pie_data_transformer(self, df, column_meta):
        ...
        for item in secondary_axis:
            ...
            for index, data_point in enumerate(internal_data[item["column_name"]]):
                data.append(
                    {
                        "name": internal_data[primary_axis["column_name"]][index],
                        "y": data_point,
                    }
                )
            pie_data_series.append({"name": item["column_name"], "data": data})
    ...
    def bubble_data_transformer(self, df, column_meta):
        ...
        if len(secondary_axis) == 3:
            ...
            for index, item in enumerate(name):
                bubble_data_series.append(
                    {"x": x[index], "y": y[index], "z": z[index], "name": name[index]}
                )
    ...
    def linear_data_transformer(self, df, column_meta, chart_type):
        ...
        for item in secondary_axis:
            if item["group_by"] and chart_type == "grouped_bar_graph":
                linear_data_series.append(
                    {
                        "name": item["column_name"],
                        "data": internal_data[item["column_name"]],
                        "stack": item["group_by"],
                    }
                )
            else:
    ...
    def grouping_data_transformer(self, df, column_meta):
        ...
        for item in secondary_axis:
            ...
            linear_data_series.append(
                {
                    "name": item["column_name"],
                    "data": [sum(internal_data[item["column_name"]])],
                }
            )
    ...
    def general_data_transformer(self, chart_type, df, column_meta, base_config):
        ...
        if chart_type == "pie_chart":
            ...
            series, categories = self.grouping_data_transformer(
                df=df, column_meta=column_meta
            )
            data_added_config["series"] = series
            ...
            series, categories = self.linear_data_transformer(
                df=df, column_meta=column_meta, chart_type=chart_type
            )
            data_added_config["series"] = series
            data_added_config["xAxis"]["categories"] = categories
    ...
    def generate_general_highcharts_config(
        self,
        chart_type,
        x_axis_title=None,
        y_axis_title=None,
    ...
    def generate_highcharts_config(self, config, data):
        ...
        transformed_internal_data = self.general_data_pre_trasformer(
            df=internal_data, column_meta=column_meta, chart_type=chart_type
        )
        series_internal_data = self.transform_to_series(df=transformed_internal_data)

        completed_config = self.general_data_transformer(
            df=series_internal_data,
