...
from livedocs.utils.cells.table_helpers import apply_filters
...
class TestApplyFilters(unittest.TestCase):
    ...
    def test_single_filter(self):
        ...
        result = apply_filters(
            self.df, [{"column": "name", "operator": "eq", "value": "Alice"}]
        )
        ...
        result = apply_filters(
            self.df, [{"column": "age", "operator": "gt", "value": 30}]
        )
        self.assertEqual(len(result), 3)  # 35, 40, 45

        # Test string contains filter
        result = apply_filters(
            self.df, [{"column": "name", "operator": "contains", "value": "a"}]
        )
        self.assertEqual(
            len(result), 2
        )  # Charlie, David (note: 'a' is case-sensitive, so Alice is not included)
    ...
    def test_edge_cases(self):
        ...
        result = apply_filters(
            self.df, [{"column": "non_existent", "operator": "eq", "value": "test"}]
        )
        self.assertEqual(
            len(result), 5
        )  # Should return original DataFrame with warning

        # Test with invalid operator
        result = apply_filters(
            self.df, [{"column": "name", "operator": "invalid_op", "value": "test"}]
        )
        self.assertEqual(
            len(result), 5
        )  # Should return original DataFrame with warning
        ...
        self.assertEqual(
            len(result), 3
        )  # Should skip invalid filter and apply valid one
    ...
    def test_all_operators(self):
        ...
        # Test eq (equality)
        result = apply_filters(
            test_df, [{"column": "value", "operator": "eq", "value": "abc"}]
        )
        self.assertEqual(len(result), 1)

        # Test contains
        result = apply_filters(
            test_df, [{"column": "value", "operator": "contains", "value": "abc"}]
        )
        self.assertEqual(len(result), 2)  # "abc", "abc123"

        # Test startsWith
        result = apply_filters(
            test_df, [{"column": "value", "operator": "startsWith", "value": "abc"}]
        )
        self.assertEqual(len(result), 2)  # "abc", "abc123"

        # Test endsWith
        result = apply_filters(
            test_df, [{"column": "value", "operator": "endsWith", "value": "def"}]
        )
        self.assertEqual(len(result), 2)  # "def", "123def"

        # Test notNull
        result = apply_filters(
            test_df, [{"column": "value", "operator": "notNull", "value": None}]
        )
        self.assertEqual(len(result), 7)  # All except None

        # Test null
        result = apply_filters(
            test_df, [{"column": "value", "operator": "null", "value": None}]
        )
        self.assertEqual(len(result), 1)  # Only None

        # Test notEmpty
        result = apply_filters(
            test_df, [{"column": "value", "operator": "notEmpty", "value": None}]
        )
        self.assertEqual(len(result), 6)  # All except empty and None

        # Test empty
        result = apply_filters(
            test_df, [{"column": "value", "operator": "empty", "value": None}]
        )
